describe("Isteven multiselect", function() {
  ...
  var $compile, $rootScope, $document, $templateCache, element, $browser, $log;
  
  beforeEach(function() {
    module('isteven-multi-select');
  });

  beforeEach(inject(function(_$compile_, _$rootScope_, _$document_, _$templateCache_, _$browser_, _$log_) {
    $compile = _$compile_;
  ...
  describe("Helper Elements", function() {
    ...
    it("by default all helper elements should show", function() {
      ...
      var element = compileDropdownHtml(dropdownHtml(), scope),
          texts = ['✓  Select All', '×  Select None', '↶  Reset'];

      //act
      scope.$apply();
      
      //assert
      var helperContainer = element.find('.helperContainer'), helperButtons = helperContainer.find('.helperButton');
      var buttonTexts = Array.from(helperButtons).map(function(item, index){
        //comparing elements
        return $(item).text().replace(/\s/g, '') == texts[index].replace(/\s/g, '');
      })
      //taking out list of all matched element
      .filter(function(isPassed){
          return isPassed;
      });

      //all three elements found
      expect(buttonTexts.length).toEqual(texts.length);
      expect(helperContainer.find('input').length).toEqual(1);
    });
  ...
  */
  ...
  /*
    ...
    i)  Callback should be functional with and without data
      ...
      f) on-reset
      g) on-clear
    ii) They should be in correct order
    iii) on-search-change should filter the inputModel.
    iv) on-search-change should pass outputmodel with settled output properties only.
  ...
  */
  ...
  /*
    ...
    ii)  selectAll button should not appeared
